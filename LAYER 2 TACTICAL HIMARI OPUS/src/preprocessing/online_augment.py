"""
HIMARI Layer 2 - Online Augmentation for Real-Time Inference
Subsystem A: Data Preprocessing (Method A8)

Purpose:
    Apply lightweight data augmentations during live inference to improve model
    robustness. Unlike offline augmentation (TimeGAN/Tab-DDPM), this runs in
    real-time with ~1ms latency.

Augmentation Types:
    1. Jitter: Small Gaussian noise to simulate market microstructure
    2. Time Warp: Subtle temporal stretching/compression
    3. Feature Masking: Random dropout to prevent over-reliance

Regime-Adaptive:
    - More augmentation during uncertain periods (high disagreement)
    - Less augmentation during clear trends (preserve signal)

Performance:
    - +0.01 Sharpe from improved robustness
...
class OnlineAugmentor:
    ...
    def __init__(self, config: Optional[OnlineAugmentConfig] = None):
        ...
        logger.debug(f"OnlineAugmentor initialized: jitter_std={self.config.jitter_std}")
